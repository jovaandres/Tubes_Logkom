...
initInventory:-
    ...
    asserta(inventory([cow, 1, 0, 1500],sell)),
...
cekBuang(Buang):- 
    inventory([Buang,Jml,_,_],_),Jml>0 -> 
        format("You have ~w ~w. How many do you want to throw? (press 0 to cancel)\n>>",[Jml,Buang]),
...
% checking item amount in inventory
seratusitem(Boolean,Tambah) :- 
    createliststuff(_,Y,_), 
    sumlist(Y,Amount),
    Amount1 is (Amount+Tambah),
    (Amount1 > 100 -> 
        Boolean is 1;
    Boolean is 0).